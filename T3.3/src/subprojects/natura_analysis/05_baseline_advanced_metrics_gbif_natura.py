"""
Script: natura_analysis/05_baseline_advanced_metrics_gbif_natura.py
...
Description:
Computes advanced analytical metrics from the baseline GBIF–Natura
joined dataset produced by the point-in-polygon workflow.

This script builds on top of the baseline basic metrics step and
focuses on three complementary analytical levels:

1) Species-level metrics (per taxonKey)
   - number of occurrences inside Natura sites
   - number of distinct Natura sites (SITECODE)
...
2) Site-type-level metrics (per SITETYPE)
   - number of Natura sites with at least one occurrence
   - number of occurrences inside Natura sites
   - number of distinct species
   - temporal coverage

3) Temporal gaps per Natura site (per SITECODE)
...
- Species-level metrics:
  results/natura_analysis/<DATASET_NAME>/
      GBIF_<DATASET_NAME>_baseline_advanced_species_metrics.csv

- Site-type-level metrics:
  results/natura_analysis/<DATASET_NAME>/
      GBIF_<DATASET_NAME>_baseline_advanced_sitetype_metrics.csv

- Site-level temporal gaps:
  results/natura_analysis/<DATASET_NAME>/
      GBIF_<DATASET_NAME>_baseline_advanced_sites_temporal_gaps.csv
...
- This script operates only on the baseline point-in-polygon output.
  Uncertainty-aware metrics should be computed separately.
"""
...
# Baseline joined GBIF–Natura dataset (output of 03_spatial_join_gbif_natura.py)
...
SPECIES_METRICS_CSV = RESULTS_DIR / f"GBIF_{DATASET_NAME}_baseline_advanced_species_metrics.csv"
SITETYPE_METRICS_CSV = RESULTS_DIR / f"GBIF_{DATASET_NAME}_baseline_advanced_sitetype_metrics.csv"
SITES_GAPS_CSV = RESULTS_DIR / f"GBIF_{DATASET_NAME}_baseline_advanced_sites_temporal_gaps.csv"

# ----------------------------------------------------------------------
# Logging setup
# ----------------------------------------------------------------------
LOG_DIR = PROJECT_ROOT / "logs" / "natura_analysis" / "05_baseline_advanced_metrics_gbif_natura"
LOG_DIR.mkdir(parents=True, exist_ok=True)

timestamp = datetime.now().strftime("%Y%m%d_%H%M")
logfile = LOG_DIR / f"baseline_advanced_metrics_{DATASET_NAME}_{timestamp}.log"

logger = logging.getLogger(f"baseline_advanced_metrics_{DATASET_NAME}")
...
# Sanity checks
# ----------------------------------------------------------------------
log("=== Baseline advanced metrics computation for GBIF–Natura joined dataset started ===")
...
if df_in_site.empty:
    log("No occurrences linked to Natura sites – no advanced metrics will be produced.")
    log("=== Baseline advanced metrics computation completed (no data) ===")
    raise SystemExit(0)
...
if TAXON_KEY_COL in df_in_site.columns:
    ...
    if has_year:
        ...
        species_metrics["temporal_completeness"] = species_metrics["n_years"] / (span + 1)
        species_metrics.loc[
            species_metrics["temporal_span"] == 0, "temporal_completeness"
        ] = 1.0

    cols_to_int = [TAXON_KEY_COL, "n_occurrences_natura", "n_sites", "n_ms"]
    if has_year:
    ...
    log("WARNING: taxonKey column not found – species-level metrics were not produced.")
...
if SITETYPE_COL in df_in_site.columns:
    ...
    cols_to_int = ["n_sites", "n_occurrences", "n_species"]
    if has_year:
        cols_to_int += ["year_min", "year_max", "n_years"]

    sitetype_metrics = cast_int_columns(sitetype_metrics, cols_to_int)
    ...
    log("WARNING: SITETYPE column not found – site-type metrics were not produced.")

# ----------------------------------------------------------------------
# 3) Temporal gaps per Natura site (per SITECODE)
# ----------------------------------------------------------------------
log("\nComputing temporal gaps per Natura site (per SITECODE)...")
...
if has_year and SITE_CODE_COL in df_in_site.columns:
    ...
    log(f"Computed temporal gap metrics for {len(sites_gaps)} Natura sites.")
    sites_gaps.to_csv(SITES_GAPS_CSV, index=False)
    log(f"Site temporal gaps saved to: {rel(SITES_GAPS_CSV)}")
else:
    if not has_year:
        log("INFO: 'year' column not available – temporal gap metrics were not produced.")
    else:
        log("WARNING: SITECODE column not found – temporal gap metrics were not produced.")

log("\n=== Baseline advanced metrics computation completed successfully ===")