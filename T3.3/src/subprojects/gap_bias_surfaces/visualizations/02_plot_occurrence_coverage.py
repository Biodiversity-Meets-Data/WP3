"""
Script: 02_plot_occurrence_coverage.py
...
03_aggregate_to_spatial_units.py and visualizes occurrence coverage
within the selected study area.

Coverage is represented as a binary spatial pattern:
- cells with at least one occurrence record
- cells with zero occurrence records

Purpose:
To provide a first spatial overview of data availability and gaps
across the selected study area.
...
OUTPUT_FILE = (
    OUT_DIR
    / f"GBIF_{DATASET_NAME}_{STUDY_AREA_NAME}_h3_coverage_res{H3_RESOLUTION}.png"
)
...
# ----------------------------------------------------------------------
# Create binary coverage column
# ----------------------------------------------------------------------
grid_gdf["has_occurrences"] = grid_gdf["n_occurrences"] > 0

n_covered = int(grid_gdf["has_occurrences"].sum())
n_empty = int((~grid_gdf["has_occurrences"]).sum())

print(f"Cells with occurrences: {n_covered}")
print(f"Cells without occurrences: {n_empty}")
...
# Create plot
# ----------------------------------------------------------------------
print("Generating occurrence coverage map...")

fig, ax = plt.subplots(figsize=(10, 10))

# Plot empty cells first
grid_gdf[~grid_gdf["has_occurrences"]].plot(
    ax=ax,
    color="lightgrey",
    edgecolor="none",
    alpha=0.6
...
ax.set_title(
    f"{DATASET_NAME} occurrence coverage ({STUDY_AREA_NAME}, res={H3_RESOLUTION})"
)
ax.set_axis_off()
...
print(f"Occurrence coverage figure saved to: {OUTPUT_FILE}")
